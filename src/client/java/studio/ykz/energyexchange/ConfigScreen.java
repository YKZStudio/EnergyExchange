...
public final class ConfigScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        addRenderableWidget(Button.builder(label("config.unknown", value.showUnlearned()), b -> {
            value = new ExchangeConfig(!value.showUnlearned(), value.compactNumbers(), value.xpEnabled(), value.xpCost()); b.setMessage(label("config.unknown", value.showUnlearned()));
        }).bounds(x, 50, 220, 20).build());
        addRenderableWidget(Button.builder(label("config.compact", value.compactNumbers()), b -> {
            value = new ExchangeConfig(value.showUnlearned(), !value.compactNumbers(), value.xpEnabled(), value.xpCost()); b.setMessage(label("config.compact", value.compactNumbers()));
        }).bounds(x, 74, 220, 20).build());
        addRenderableWidget(Button.builder(label("config.xp", value.xpEnabled()), b -> {
            value = new ExchangeConfig(value.showUnlearned(), value.compactNumbers(), !value.xpEnabled(), value.xpCost()); b.setMessage(label("config.xp", value.xpEnabled()));
        }).bounds(x, 110, 220, 20).build());
        xpCost = addRenderableWidget(new EditBox(font, x, 154, 220, 20, Messages.text("config.xp_cost")));
        xpCost.setMaxLength(128); xpCost.setValue(value.xpCost());
        addRenderableWidget(Button.builder(Messages.text("config.save"), b -> {
            try { new ExchangeConfig(value.showUnlearned(), value.compactNumbers(), value.xpEnabled(), xpCost.getValue()).save(); onClose(); }
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g, int x, int y, float delta) {
        ...
        g.text(font, Messages.text("config.xp_cost"), width / 2 - 110, 140, 0xFFFFFFFF);
        g.textWithWordWrap(font, Messages.text("config.server_note"), width / 2 - 140, 184, 280, 0xFFAAAAAA);
